        ...
        if(args .length < 2){
            if(cs instanceof ProxiedPlayer){
                ...
                if(RankSystem.hasMod(UUIDFetcher.getUUID(p.getName()))){
                    p.sendMessage(new TextComponent(BanSystem.prefix + "Verwendung: §6/editban <Spieler> <Grund>"));
                } else {
                    p.sendMessage(new TextComponent(BanSystem.prefix + "§cKeine Rechte!"));
                }
            } else {
                ProxyServer.getInstance().getConsole().sendMessage(new TextComponent(BanSystem.prefix + "Verwendung: §6/editban <Spieler> <Grund>"));
            }
        } else {
            if(cs instanceof ProxiedPlayer){
                ProxiedPlayer p = (ProxiedPlayer)cs;
                        ...
                        if(MySQL.isCurrentlyMuted(uuid)){
                            ...
                            for(ProxiedPlayer all : ProxyServer.getInstance().getPlayers()){
                                if(RankSystem.hasMod(UUIDFetcher.getUUID(all.getName()))){
                                    all.sendMessage(new TextComponent(BanSystem.prefix + p.getDisplayName() + " §7hat den Mutegrund von §a" + args[0] + " §7zu §e" + reason + " §7umgeändert!"));
                                }
                            }
                        } else {
                ...
                if(uuid != null){
                    if(MySQL.isCurrentlyBanned(uuid)){
                        ...
                        for(ProxiedPlayer all : ProxyServer.getInstance().getPlayers()){
                            if(RankSystem.hasMod(UUIDFetcher.getUUID(all.getName()))){
                                all.sendMessage(new TextComponent(BanSystem.prefix +"§4BungeeConsole §7hat den Mutegrund von §a" + args[0] + " §7zu §e" + reason + " §7umgeändert!"));
                            }
                        }
                    } else {
